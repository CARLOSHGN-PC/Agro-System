...
/**
 * Escuta mudanças em tempo real na coleção de Insumos.
 */
        ...
        if (hasChanges) {
            try {
                if (toAddOrUpdate.length > 0) {
                    await db.insumos.bulkPut(toAddOrUpdate);
                }
                if (toDeleteIds.length > 0) {
                    await db.insumos.bulkDelete(toDeleteIds);
                }
            } catch (err) {
                console.error("[Insumos Realtime] Erro ao sincronizar para o Dexie:", err);
            }
        }
    });
